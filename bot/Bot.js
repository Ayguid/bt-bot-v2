...
class TradingBot {
    ...
    */
    ...
    buyInPrice() {
        ...
        return minusPercent(this.precisionEntry || this.currentPair.belowPrice, this.currentPrice);
    }


    calculatePrecisionEntry() {
        // Fallback if no order book data
        if (!this.currentOrderBook?.asks?.[0]) {
            return Math.min(Math.max(this.currentPair.belowPrice || 0.1, 0.1), 1.0);
        }
    
        // 1. Get spread and volatility
        ...
        const volatility = this.getVolatilityAssessment(this.currentOhlcvPrimary);
    
        // 2. Calculate base distance (combine volatility + pair config)
        const baseDistance = this.currentPair.belowPrice || 0.1;
        const dynamicBase = baseDistance * (1 + (volatility / 100));
    
        // 3. Apply spread protection and bounds
        let finalDistance = Math.max(
            dynamicBase,
            spreadPercentage * 1.5, // Ensure fillability
            0.1 // Absolute minimum
        );
        finalDistance = Math.min(finalDistance, 1.0); // Absolute maximum
    ...
    async handleFilledOrder(lastOrder, shouldBuy, shouldSell) {
        ...
        if (lastOrder.side === TradingBot.BUY) {
            ...
            if (shouldTakeProfit) console.log('Selling due to profit target');
            if (shouldCutLosses) console.log('Selling due to stop loss');
            if (strongSellSignal) console.log('Selling due to strong sell signal');
            //if (candlePatternScore < -0.5) console.log('Selling due to bearish candle pattern');
        } 
...
//         this.orderState = {}; // Local order state by pair key
//         this.lastOrderSync = {}; // Last sync time by pair key
//     }
...
//         if (current[4] > current[1] && bodyRatio > 0.7) return 0.8;
//         if (current[4] < current[1] && bodyRatio > 0.7) return -0.8;
//         return 0;
//     }
...
//             return this.currentPair.belowPrice || 0.2;
//         }
...
//         minDistance = Math.min(minDistance, 1.0);
//         minDistance = Math.max(minDistance, this.currentPair.belowPrice || 0.2, 0.1);
...
//                 if (sellOrder) {
//                     this.orderState[this.currentPair.key].push(sellOrder);
//                 }
//                 return sellOrder;
//             }
//         } 
...
//             if (orderPriceDiff >= this.currentPair.profitMgn || !shouldBuy) {
//                 console.log(`Cancelling buy order (Price diff: ${orderPriceDiff.toFixed(2)}%)`);
//                 const canceledOrder = await this.exchangeManager.cancelAndSellToCurrentPrice(
//                     this.currentPair, 
//                     lastOrder, 
//                     this.currentPrice, 
//                     true
...
//                 if (canceledOrder) {
//                     this.orderState[this.currentPair.key] = this.orderState[this.currentPair.key].map(o => 
//                         o.orderId === canceledOrder.orderId ? canceledOrder : o
//                     );
//                 }
...
//             if (!shouldBuy || orderPriceDiff >= this.currentPair.okDiff) {
//                 console.log(`Cancelling Buy Order (Price diff: ${orderPriceDiff.toFixed(2)}%)`);
//                 const canceledOrder = await this.exchangeManager.cancelOrder(this.currentPair, lastOrder);
...
//         const [ohlcvPrimary, ohlcvSecondary, , orderBook] = await this.exchangeManager.fetchPairData(
//             pair,
//             this.config.klinesInterval_1,
//             this.config.klinesInterval_2
//         );
...
//         if (!this.orderState[pair.key]) {
//             try {
//                 const orders = await this.exchangeManager.makeQueuedReq(fetchMyOrders, pair.joinedPair);
//                 this.orderState[pair.key] = orders || [];
//                 this.lastOrderSync[pair.key] = Date.now();
//                 console.log(`Fetched initial orders for ${pair.key}:`, this.orderState[pair.key].length);
//             } catch (error) {
//                 console.error(`Error fetching initial orders for ${pair.key}:`, error);
//                 this.orderState[pair.key] = [];
//             }
//         }
//         this.currentOrders = this.orderState[pair.key];
...
//     async syncOrdersIfNeeded(pair) {
//         const lastSync = this.lastOrderSync[pair.key] || 0;
...
//                 this.orderState[pair.key] = orders || [];
//                 this.lastOrderSync[pair.key] = Date.now();
//                 console.log(`Synced orders for ${pair.key}`);
//             } catch (error) {
//                 console.error(`Error syncing orders for ${pair.key}:`, error);
//             }
//         }
//     }
// }
